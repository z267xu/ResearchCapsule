...
| Symbol | Meaning |
|---|---|
| $W$ | Treatment (e.g. question-mark headline vs not). |
| $Y$ | Human outcome (e.g. CTR). The ATE we actually want. |
| $Y^*$ | LLM-generated outcome for the same unit / item. |
| $X$ | Covariates. |
| $\mu(X, Y^*)$ | Calibration function: maps LLM output to the human-outcome scale. Fit on a human sample. |
| $K$ | Independent LLM draws per unit. Averaging $K$ draws reduces attenuation. |
...
- Connects to **Section 6.4 (long-term outcomes)**: calibrating on a short-term pilot when the target is a steady-state/long-term effect requires a **two-step surrogate chain** ($Y^* \to Y_{\mathrm{short}} \to Y_{\mathrm{long}}$), and novelty effects break the second link.
